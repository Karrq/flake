  ...
  outputs = inputs@{ self, flake-parts, nixpkgs, ... }:
    flake-parts.lib.mkFlake { inherit inputs; } ({ withSystem, ... }: {
      ...
      perSystem = { config, self', inputs', system, pkgs, ... }:
        let
          dockerized = if pkgs.stdenv.isDarwin then
            self.packages."${builtins.replaceStrings [ "darwin" ] [ "linux" ]
            system}".docker
          else
          ...
          appDev = config.packages.default.overrideAttrs
            (_: _: { noDevDependencies = false; });
          shell = pkgs.mkShell {
            inputsFrom = [ appDev ];
            shellHook = ''
